package hiennguyen.me.architecture.rvbindinglib.helper;
...
public class ComputingAdapterChangedHelper<T> {
    ...
    public ComputingAdapterChangedHelper(RecyclerView.Adapter adapter, DiffCallback<T> diffCallback, boolean changeOnMainThread ) {
        ...
        if(changeOnMainThread) {
            builder.setBackgroundThreadExecutor(command -> new Handler(Looper.getMainLooper()).post(command));
        }
